...
function LaserClass:update(dt)
    if kd(" ") then
         ...
         local shootAudio = love.audio.newSource("shoot.mp3")
        ...
        while self.cooloff >= 0 do
            ...
            elseif weapon_mode == 3 then -- level 3: Powerful 3-way beam
            ...
            else -- level 4: 8-way beam
                local laserM1 = LaserBeam(player.x - 10, player.y, 0)
                local laserM2 = LaserBeam(player.x + 10, player.y, 0)
                local laserL1 = LaserBeam(player.x - 40, player.y, -500)
                local laserL2 = LaserBeam(player.x - 20, player.y, -500)
                local laserL3 = LaserBeam(player.x - 40, player.y, -1500)
                local laserL4 = LaserBeam(player.x - 60, player.y, -1500)
                local laserR1 = LaserBeam(player.x + 20, player.y, 500)
                local laserR2 = LaserBeam(player.x + 40, player.y, 500)
                local laserR3 = LaserBeam(player.x + 40, player.y, 1500)
                local laserR4 = LaserBeam(player.x + 60, player.y, 1500)
                addActor(laserM1)
                addActor(laserM2)
                addActor(laserL1)
                addActor(laserL2)
                ...
                laserL1:update(self.cooloff)
                laserL2:update(self.cooloff)
                laserL3:update(self.cooloff)
                laserL4:update(self.cooloff)
                laserR1:update(self.cooloff)
                laserR2:update(self.cooloff)
                laserR3:update(self.cooloff)
                laserR4:update(self.cooloff)
            end
